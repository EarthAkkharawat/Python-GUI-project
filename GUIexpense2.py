...
L = ttk.Label(GUI,text='Expense').place(x=100,y=270)
L = ttk.Label(GUI,text='Income').place(x=200,y=270)
L = ttk.Label(GUI,text='Remaining').place(x=300,y=270)
...
def remaining():
	...
	v_remaining.set('{:,.2f} baht'.format(sum_remaining))
...
def sum_table2():
	...
	for dt in data:
		allsum.append(float(dt[2]))
	v_allincome.set('{:,.2f} baht'.format(sum(allsum)))
...
def SaveIncome(event=None):
	...
	Writetocsv2(data) #call saving function.
...
table_income = ttk.Treeview(T2, column = header, show = 'headings', height = 20)
...
L = ttk.Label(T3,text = 'ยอดรายจ่ายทั้งหมด (บาท):',font=FONT1,foreground='orange').place(x=50,y=50)
LR1 = ttk.Label(T3, textvariable=v_allexpense,font=FONT2,foreground='orange')
LR1.place(x=400,y=50)

v_allincome = StringVar()

L = ttk.Label(T3,text = 'ยอดรายรับทั้งหมด (บาท):',font=FONT1,foreground='green').place(x=50,y=100)
LR2 = ttk.Label(T3,textvariable=v_allincome,font=FONT2,foreground='green')
LR2.place(x=400,y=100)

v_remaining = StringVar()

L = ttk.Label(T3,text = 'คงเหลือทั้งหมด (บาท):',font=FONT1,foreground='blue').place(x=50,y=150)
LR2 = ttk.Label(T3,textvariable=v_remaining,font=FONT2,foreground='blue')
